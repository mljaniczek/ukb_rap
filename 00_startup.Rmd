...
Use HIGH PRIORITY (otherwise it will keep quitting and exiting and you lose all your updated code!!!)
...
And then paste in your github PAT token. 

Then back in terminal, clone your github repo with code for your project:
git clone https://github.com/mljaniczek/ukb_rap 

THEN make sure you open project (upper right corner), otherwise you won't be able to easily push your changes back to github! 

Now you're ready to actually code!!
...
## Narrow down list of fields

To avoid downloading all 35K fields you probably want to narrow down your scope. Ahead of time for your analysis, identify which fields you need for your subset of data. 

You may also want to narrow down your cohort if you are not going to analyze all 500K UKB subjects, do that in Cohort Browser, separately. 

Below I have loaded in the fields available in my dataset, then filter down to make a few .txt files specific to the three separate data types that I use in my analysis. 

1. Phenotype data (age, BMI, smoking status, menopausal status etc)
- make pheno.txt, which should be a ONE COLUMN with the field names you want. Make sure to include the eid as well which will id the subject!! 
...
```{r}
pheno_text <- field_dx_select %>%
  mutate(text_column = str_replace_all(title, "[^[:alnum:]]", ".")) %>%
  left_join(char_key, by = c("field_mod3" = "field.showcase")) %>%
  select(name, title, text_column, field_mod3, col.name)
```
