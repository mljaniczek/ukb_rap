...
tabdat <- dat_sub_metab2 %>%
  dplyr::select(sex_f31_0_0, age_cat, menopause_cat, race, current_tobacco_smoking_f1239_0_0, metabolite_status, BMI= body_mass_index_bmi_f21001_0_0)

var_label(tabdat) <- list("sex_f31_0_0" = "Sex",
...
metab_tab_phase1_50 <- tabdat %>%
  ...
  dplyr::select(sex_f31_0_0, BMI, race, current_tobacco_smoking_f1239_0_0) %>%
  tbl_summary(by = sex_f31_0_0, missing = "no") %>%
  bold_labels() %>%
  #add_n() %>%
  add_overall()
...
tbl_strata1 <- tabdat %>%
  ...
  dplyr::select(sex_f31_0_0, age_cat, race, BMI, current_tobacco_smoking_f1239_0_0) %>%
  tbl_strata(
    strata = age_cat,
    .tbl_fun = 
      ~.x %>%
...
tabdat_age <- dat_sub_metab2 %>%
  dplyr::select(age_cat, sex_f31_0_0, metabolite_status, age_when_attended_assessment_centre_f21003_0_0)
...
met_long <- dat_phase1 %>%
  dplyr::select(eid, age_cat, age =age_when_attended_assessment_centre_f21003_0_0, menopause_cat, sex = sex_f31_0_0, all_of(met_key$col.name)) %>%
...
met_long3_p2 <- dat_phase2 %>%
  dplyr::select(eid, age_cat, age = age_when_attended_assessment_centre_f21003_0_0, race, SMOKING,
                 menopause_cat, sex = sex_f31_0_0, BMI, all_of(met_key$col.name)) %>%
  pivot_longer(-c(eid, age_cat, age, race, SMOKING, menopause_cat, sex, BMI), names_to = "metabolite", values_to = "value")
...
met_long3a <- dat_phase1 %>%
  ...
  dplyr::select(eid, age_cat,age = age_when_attended_assessment_centre_f21003_0_0, race, smoker = current_tobacco_smoking_f1239_0_0, menopause_cat, sex = sex_f31_0_0, all_of(metab_key2$col.name), waist_hip_ratio) %>%
  pivot_longer(-c(eid, age_cat, age, race, smoker, menopause_cat, sex, waist_hip_ratio), names_to = "metabolite", values_to = "value")

long_test_7 <- met_long3a %>%
  filter(age_cat %in% c("<50", "50-59")) %>%
  drop_na()%>% 
  nest(.by = c(age_cat, metabolite)) %>%
  mutate(model2 = map(data, ~lm(value ~ sex + menopause_cat + age + smoker + race + waist_hip_ratio, data = .x)),
         tidied2 = map(model2, ~tidy(.x, conf.int = T)))%>%
  dplyr::select(-data)
...
long_test_8 <- met_long3a %>%
  ...
  nest(.by = c(age_cat, metabolite)) %>%
  # set up both outcome model (model_y) and mediator model (model_m)
  mutate(model2 = map(data, ~lm(value ~ sex +age + smoker + race + waist_hip_ratio, data = .x,
                                 x = FALSE, y = FALSE, model = FALSE)),
         tidied2 = map(model2, ~tidy(.x, conf.int = T))) %>%
  dplyr::select(-data)
...
met_long3a_p2 <- dat_phase2 %>%
  mutate(waist_hip_ratio = waist_circumference_f48_0_0/hip_circumference_f49_0_0) %>%
  dplyr::select(eid, age_cat, age = age_when_attended_assessment_centre_f21003_0_0, race, smoker = current_tobacco_smoking_f1239_0_0,
                 menopause_cat, sex = sex_f31_0_0, waist_hip_ratio, all_of(metab_key2$col.name)) %>%
  pivot_longer(-c(eid, age_cat, age, race, smoker, menopause_cat, sex, waist_hip_ratio), names_to = "metabolite", values_to = "value")
...
Now trying to see if using both BMI and hip/waist ratio affects things.
Adjust for: 
...
met_long4 <- dat_phase1 %>%
    mutate(waist_hip_ratio = waist_circumference_f48_0_0/hip_circumference_f49_0_0) %>%
  dplyr::select(eid, age_cat,age = age_when_attended_assessment_centre_f21003_0_0, race, smoker = current_tobacco_smoking_f1239_0_0, menopause_cat, sex = sex_f31_0_0, all_of(metab_key2$col.name), waist_hip_ratio, bmi = body_mass_index_bmi_f21001_0_0) %>%
  pivot_longer(-c(eid, age_cat, age, race, smoker, menopause_cat, sex, waist_hip_ratio, bmi), names_to = "metabolite", values_to = "value")

long_test_4a <- met_long4 %>%
  filter(age_cat %in% c("<50", "50-59")) %>%
  drop_na()%>% 
  nest(.by = c(age_cat, metabolite)) %>%
  mutate(model2 = map(data, ~lm(value ~ sex + menopause_cat + age + smoker + race + waist_hip_ratio + bmi, data = .x)),
         tidied2 = map(model2, ~tidy(.x, conf.int = T)))%>%
  dplyr::select(-data)

long_test_4b <- met_long4 %>%
  filter(age_cat %in% c("60-69", "70-85")) %>%
  drop_na() %>%
  nest(.by = c(age_cat, metabolite)) %>%
  # set up both outcome model (model_y) and mediator model (model_m)
  mutate(model2 = map(data, ~lm(value ~ sex +age + smoker + race + waist_hip_ratio + bmi, data = .x,
                                 x = FALSE, y = FALSE, model = FALSE)),
         tidied2 = map(model2, ~tidy(.x, conf.int = T))) %>%
  dplyr::select(-data)


res_lt50_to_59_M4 <- long_test_4a %>% dplyr::select(metabolite, age_cat, tidied2) %>%
  unnest(tidied2) %>%
  filter(term == "sex.L")

res_60_to_85_M4 <- long_test_4b %>% dplyr::select(metabolite, age_cat, tidied2) %>%
  unnest(tidied2) %>%
  filter(term == "sex.L")

all_strat_M4 <- bind_rows(res_lt50_to_59_M4, res_60_to_85_M4) %>%
  mutate(fdr = p.adjust(p.value, "fdr"),
         phase = "Phase I",
         met_threshold_m4 = fdr < 0.05 & abs(estimate)>0.25) 

# data.frame(
#   M3_estimates = all_strat_M3$estimate,
#   M3a_estimates = all_strat_M3a$estimate,
#   M4_estimates = all_strat_M4$estimate
# ) %>%
#   ggplot(aes(x = M3_estimates, y = M3a_estimates)) +
#   geom_point( aes(alpha = .5)) +
#   geom_smooth(method=lm) +
...
# looking at correlation between WHR and BMI
data.frame(
  waist_hip_ratio = dat_phase1$waist_circumference_f48_0_0/dat_sub_metab2$hip_circumference_f49_0_0,
  bmi = dat_sub_metab2$body_mass_index_bmi_f21001_0_0
) %>%
...
subset_M4 <- all_strat_M4 %>%
  filter(fdr < 0.05 & abs(estimate)>.4) %>%
  mutate(phase = "Phase I",
         phase_mod = "Phase I M4")


met_long4_p2 <- dat_phase2 %>%
  mutate(waist_hip_ratio = waist_circumference_f48_0_0/hip_circumference_f49_0_0) %>%
  dplyr::select(eid, age_cat, age = age_when_attended_assessment_centre_f21003_0_0, race, smoker = current_tobacco_smoking_f1239_0_0,
                 menopause_cat, sex = sex_f31_0_0, waist_hip_ratio, bmi = body_mass_index_bmi_f21001_0_0, all_of(metab_key2$col.name)) %>%
  pivot_longer(-c(eid, age_cat, age, race, smoker, menopause_cat, sex, waist_hip_ratio, bmi), names_to = "metabolite", values_to = "value")

long_test_4a_p2 <- met_long4_p2 %>%
  filter(age_cat %in% c("<50", "50-59")) %>%
  drop_na()%>% 
  nest(.by = c(age_cat, metabolite)) %>%
  mutate(model2 = map(data, ~lm(value ~ sex + menopause_cat + age + smoker + race + waist_hip_ratio + bmi, data = .x)),
...
res_lt50_to_59_M4_p2 <- long_test_4a_p2 %>% dplyr::select(metabolite, age_cat, tidied2) %>%
  unnest(tidied2) %>%
  filter(term == "sex.L")

res_60_to_85_M4_p2 <- long_test_4b_p2 %>% dplyr::select(metabolite, age_cat, tidied2) %>%
  unnest(tidied2) %>%
  filter(term == "sex.L")

all_strat_M4_p2 <- bind_rows(res_lt50_to_59_M4_p2, res_60_to_85_M4_p2) %>%
  mutate(fdr = p.adjust(p.value, "fdr"),
         phase = "Phase II",
         met_threshold_m4_p2 = fdr < 0.05 & abs(estimate)>0.25) 

subset_M4_p2 <- all_strat_M4_p2 %>%
  filter(fdr < 0.05 #& abs(estimate)>.25
         ) %>%
  mutate(phase = "Phase II",
         phase_mod = "Phase II M4")

```

## Summarizing M4 Results
...
all_strat_both_phases_wide_M4 <- left_join(
  all_strat_M4 %>% dplyr::select(metabolite, age_cat, met_threshold_m4, fdr),
  all_strat_M4_p2 %>% dplyr::select(metabolite, age_cat, met_threshold_m4_p2)) %>%
  mutate(sig_category_m4 = ifelse(
    met_threshold_m4 == FALSE & met_threshold_m4_p2 == FALSE, "Didn't meet threshold",
    ifelse( met_threshold_m4 == TRUE & met_threshold_m4_p2 == FALSE, "Not Validated",
            ifelse(
              met_threshold_m4 == TRUE & met_threshold_m4_p2 == TRUE, "Validated", "Didn't meet threshold"
            ))
  ))

summary(as.factor(all_strat_both_phases_wide_M4$sig_category_m4))
...
res_M4 <- all_strat_M4 %>%
  left_join(all_strat_both_phases_wide_M4 %>% dplyr::select(metabolite, age_cat, sig_category_m4))%>%
  left_join(age_tab4) %>%
  mutate(
    age_obs = paste0(age_cat, " (n=", n, ")")
  )
...
# combine p1 and p2 full data
all_strat_both_phases_long_m4 <- bind_rows(res_M4, all_strat_M4_p2) %>%
  dplyr::select(metabolite, age_cat, age_obs, phase, sig_category_m4, estimate, conf.low, conf.high, fdr, met_threshold_m3a_p1 = met_threshold_m4, met_threshold_m4_p2)

save(all_strat_both_phases_long_m4, file = "results_mod4_bothphases.Rdata")
#load("results_mod3a_bothphases.Rdata")
write.csv(all_strat_both_phases_long_m4, file = "results_mod4_bothphases.csv")

# volcano plot
ggplot(res_M4, aes(x = estimate, y = -log10(fdr), color = sig_category_m4, alpha = 0.1)) +
  geom_point() +
  scale_color_manual(values = c("gray", "black", "blue"), name = "")+
  facet_wrap(~age_obs) +
  guides(alpha = FALSE) +
  geom_hline(yintercept = -log10(1.05), linetype = "dashed", color = "red") +
  xlab("Model 4 Sex Coefficient from Phase I") +
  theme_bw() +
  theme(legend.position = "right")
#ggsave("m2_volcano.png", height = 4, width = 6)
...
res_M4 %>%
  tbl_cross(row = age_obs, col = sig_category_m4)


original_sig_mets_m3 <- res_M3 %>%
  filter(met_threshold_m3 == TRUE)

validated_mets_m4 <- res_M4 %>%
  filter(fdr <0.05 #& abs(estimate) >0.25
         )
...
netdat_70 <- dat_phase1 %>%
  ...
  dplyr::select(-age_cat) %>%
  drop_na()
sex_groups <- sort(unique(dat_phase1$sex_f31_0_0))
...
netdat_all <- dat_phase1 %>%
  dplyr::select(age_cat, sex = sex_f31_0_0, all_of(validated_mets_m3$metabolite)) %>%
  drop_na() %>%
  nest(.by = c(age_cat)) %>%
