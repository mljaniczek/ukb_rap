...
final_fo <- final_fo %>%
  mutate(
    prevalent_hypertension = ifelse(hypertension == 1 & beforeassessment_fo == TRUE, 1, 0),
    prevalent_cad = ifelse(cad == 1 & beforeassessment_fo == TRUE, 1, 0),
    prevalent_diabetes_t2 = ifelse(diabetes_t2 == 1 & beforeassessment_fo == TRUE, 1, 0),
    incident_cad = ifelse(cad == 1 & afterassessment_fo == TRUE, 1, 0)
  )

write.csv(final_fo, file="first_occurrences_processed.csv")
